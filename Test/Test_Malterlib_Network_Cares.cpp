...
namespace
{
	bool fg_IsPortConflict(NStr::CStr const &_Error)
	{
		// CExceptionNet carries the formatted diagnostic; match only the platform's bind-conflict errors.
#ifdef DPlatformFamily_Windows
		for (auto Error : {WSAEADDRINUSE, WSAEACCES})
		{
			if (_Error == NStr::fg_Format("Could not bind socket, windows returned: {}", NPlatform::fg_Win32_GetLastErrorStr(Error)))
				return true;
		}

		return false;
#else
		return _Error == NPlatform::fg_FormatErrno("bind (listen)", EADDRINUSE);
#endif
	}
	...
	struct CNameServer : CActor
	{
		...
		CNameServer(EMode _Mode, TCPromise<void> const &_Started, uint16 _FirstPort = 0)
			: m_Started(_Started)
			, m_Mode(_Mode)
			, m_FirstPort(_FirstPort)
		{
		}

		CNetAddress f_Address() const
		{
			return CNetAddressTCPv4(CNetAddressIPv4(127, 0, 0, 1), uint16(m_Listen.f_GetListenPort()));
		}
		...
		{
			// TCP and UDP allocate ports independently. Retain a candidate only when both binds succeed.
			for (umint Attempt = 0; Attempt < 16; ++Attempt)
			{
				// TCP-assigned ports can cluster inside a UDP-excluded range on Windows.
				uint16 Port = Attempt ? uint16(49152 + NMisc::fg_GetRandomUnsigned() % 16384) : m_FirstPort;
				try
				{
					m_Listen.f_Listen(CNetAddressTCPv4(CNetAddressIPv4(127, 0, 0, 1), Port), f_Callback(), ENetFlag_None);
					Port = uint16(m_Listen.f_GetListenPort());
					m_UDP.f_ListenDatagram(CNetAddressTCPv4(CNetAddressIPv4(127, 0, 0, 1), Port), f_Callback(), ENetFlag_None);

					return;
				}
				catch (CExceptionNet const &_Error)
				{
					DMibLog(Debug, "DNS test server bind attempt {} failed on port {}: {}", Attempt + 1, Port, _Error.f_GetErrorStr());
					if (Attempt == 15 || !fg_IsPortConflict(_Error.f_GetErrorStr()))
						throw;
				}

				m_UDP.f_CloseAsync({});
				m_Listen.f_CloseAsync({});
			}
		}
	...
	struct CCares_Tests : NTest::CTest
	{
		void f_DoTests()
		{
			DMibTestSuite("PortCollision") -> TCFuture<void>
			{
#ifdef DPlatformFamily_Windows
				for (bool bUDP : {false, true})
#else
				for (bool bUDP : {false})
#endif
				{
					DMibTestPath(bUDP ? "UDP" : "TCP");
					CSocket Blocker;
					auto CloseBlocker = co_await fg_AsyncDestroy
						(
							[&Blocker]() -> TCFuture<void>
							{
								TCPromise<void> Closed;
								Blocker.f_CloseAsync([Closed]() mutable { Closed.f_SetResult(); });
					...
					;
					if (bUDP)
						Blocker.f_ListenDatagram(CNetAddressTCPv4(CNetAddressIPv4(127, 0, 0, 1), 0), [](ENetTCPState) {}, ENetFlag_None);
					else
						Blocker.f_Listen(CNetAddressTCPv4(CNetAddressIPv4(127, 0, 0, 1), 0), [](ENetTCPState) {}, ENetFlag_None);
					auto Port = uint16(Blocker.f_GetListenPort());

					TCPromise<void> Started;
					auto Server = fg_ConstructActor<CNameServer>(CNameServer::EMode::mc_Reply, Started, Port);
					auto DestroyServer = co_await fg_AsyncDestroy(Server);
					auto Address = co_await Server.f_Bind<&CNameServer::f_Address>();
					DMibExpect(Address.f_GetPort(), !=, Port);
					...
					auto Lookup = co_await Resolver.f_Bind<&CResolveActor::f_ResolveHost>(NStr::CStr("collision.test."), ENetAddressType_TCPv4);
					auto Addresses = co_await fg_Move(Lookup.m_Result).f_Timeout(5.0, "DNS server did not recover from a port collision");
