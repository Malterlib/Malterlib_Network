...
namespace
{
	...
	// A readiness report is requested right after a receive that would block. Data sent by the
	// peer in the moment between that request and the loop arming its interest has to be
	// reported all the same: the loop's arm is level, so what is already readable completes it
	struct CReadinessRace_Tests : public NMib::NTest::CTest
	{
		void fp_Test(CNetAddress const &_ListenAddress, bool _bTcp)
		{
			...
			for (umint iRound = 0; iRound < c_nRounds && nMisses < 5; ++iRound)
			{
				...
				// Drain to a would-block, which is where the next readiness report gets requested,
				// and clear the state the earlier reports left
				uint8 Buffer[256];
				...
				// The peer's byte lands while the loop is still arming for the request
