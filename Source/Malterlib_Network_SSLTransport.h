...
namespace NMib::NNetwork
{
	...
	struct CSSLTransport
	{
		...
		// The pieces the queue holds on to, consumed ones included: never more than the
		// capacity, since an append past it compacts the live pieces into one first
		...
		// The most pieces the inbound ciphertext is held in. The callers keep the live pieces
		// to two or three — a completion delivery that completes no record compacts, and a
		// readiness read is only issued once nothing complete is queued — and the consumed
		// ones are reclaimed once they outnumber the live ones, so this is never reached by
		// them; an append that does find the array full compacts the live pieces into one, so
		// the bound holds however the transport is driven
		static constexpr umint mc_nCipherQueueCapacity = 8;

		// The most ciphertext pieces one decrypt call is offered
	...
	protected:
		...
		// advances the head piece in place, which is all the carry there is. The live pieces
		// are the slots from mp_iCipherHead up to mp_nCipherQueue; the ones before the head
		// are consumed and reclaimed once they outnumber the live ones
		CCipherSegment mp_CipherQueue[mc_nCipherQueueCapacity];
