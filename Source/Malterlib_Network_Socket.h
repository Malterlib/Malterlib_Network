...
namespace NMib::NNetwork
{
	...
	struct ICSocketCompletionIo
	{
		...
		// False is terminal: this socket will accept no further send, and has already put itself
		// in a state the caller's close path observes. There is no "not now" — an implementation
		// that could accept the operation later has to accept it now and report through the
		// completion instead, because nothing wakes the caller to try again.
		//
		...
		virtual bool f_SubmitSendVectored(NSys::CIoSpan const *_pSpans, umint _nSpans, NSys::FIoCompletion &&_fOnComplete, FSocketSendReleased &&_fOnReleased) = 0;
