...
namespace NMib::NNetwork
{
	...
	// The generation has left whole — an operation finishes every byte it scheduled or fails —
	// so it is drained here. The pin stays: it is released by the operation's buffer-released
	// notification, which for a zero copy send comes only when the peer has acknowledged the
	// pages — until then the buffer may be neither written nor refilled
	...
	// Appends at the tail. A full array first gives up its consumed slots, and failing
	// that compacts the live pieces into one: the copy is at most an incomplete record and
	// one delivery, and no caller drives the queue that far
	...
	// Moves the live pieces down over the consumed slots
	...
	// Copies the live pieces into one owned piece and drops the rest, references included
	...
	void CSSLTransport::f_ConsumeCipher(umint _nBytes)
	{
		...
		{
			// The consumed pieces keep their slots until the queue empties, and a stream
			// whose records keep straddling its pieces never empties it: the slots would
			// run out and every append would then compact. Once the consumed slots
			// outnumber the live ones they are reclaimed, at an amortized constant cost
			fp_ReclaimCipherConsumed();
		}
	}
