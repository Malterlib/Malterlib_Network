...
namespace NMib::NNetwork
{
	...
	public:
		...
		bool f_Shutdown()
		{
			...
			// would be waiting for a shutdown that was produced and never written. Under completion
			// sends the submitted operations are the only writer, and the caller's drain carries
			// the alert out like any sealed record
		...
		bool f_TryOpenInto(void *_pData, umint _nLen, CSocketOperationResult &o_Result)
		{
			...
			// The record layer is done for good: the error it queued is what the connection
			// reports, taken now since the queue is the thread's. Plaintext still held was
			// authenticated before the failure and is delivered first, since the failed state
			// ends reads: the state is set once the hold has drained
			auto fReadFailed = [&]
				{
			...
			// A close alert opened in the same call as the last application records is recorded
			// with them: its ciphertext is gone from the transport, so nothing would show it
			// again. Plaintext still held waits it out, since the shutdown state ends reads: it
			// is set once the hold has drained
			auto fShutdown = [&]
				{
					if (mp_Transport.f_GetHeld())
			...
			for (;;)
			{
				...
				if (nFragments)
				{
					...
					// What a call opened is delivered whichever way the call ended: those records
					// have moved the read sequence and cannot be opened again by any other path
					if (nProduced)
					{
					...
					if (mp_Transport.f_GetCipherPending() > SSL3_RT_HEADER_LENGTH)
					{
						...
						// Delivered, failed and shut down in the same order as above
						if (nHeld)
						{
				...
				{
					// The transport ended with no close alert ahead of it. Only close_notify
					// authenticates an end, so this is truncation, and it fails the read the way
					// the library's own read does
					if (fp_CheckTransportError(EState_ReadFailed))
						DMibErrorNet((NStr::CStr::CFormat("Could not read from socket (SSL): {}") << mp_LastError).f_GetStr());
	...
	protected:
		...
		// A close alert or a record failure opened behind plaintext still held: the shutdown or
		// failed state waits for the hold to drain, or the rest of that record would never be read
		bool mp_bShutdownAfterHeld = false;
