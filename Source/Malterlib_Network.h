...
// Addresses
	...
	// Makes the close a reset: the kernel discards what is queued instead of retransmitting it
	// to the peer, and lets go of every zero copy page at once. Nothing to discard on a unix
	// socket, where the call does nothing
	...
	// Answers the bytes scheduled, 0 for a refusal; see ICIoLoop::f_SubmitSendVectored
...
namespace NMib::NNetwork
{
	...
	public:
		...
		// The settings travel with the socket they were made for: set before the platform socket
		// exists, they would otherwise be lost to a move
