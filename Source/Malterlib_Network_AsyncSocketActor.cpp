...
namespace NMib::NNetwork
{
	...
	// The window a connection begins at: one frame of the fragmentation size. The path grows it
	// toward the ceiling only when its bandwidth-delay product asks
	...
	NConcurrency::TCFuture<void> CAsyncSocketActor::fp_Destroy()
	{
		...
		// A destroy tears the connection down at once, whatever is still on its way to the peer
		// included: the close resets, so the peer sees the connection fail rather than close,
		// and the kernel lets go of the zero copy pages instead of retransmitting them, which
		// with an unresponsive peer would keep the fence below waiting for the retransmission
		// lifetime. That is the contract; a connection that should close cleanly shuts down and
		// drains before it destroys. Do not make the destroy wait for the peer
	...
	void CAsyncSocketActor::CInternal::f_ShutdownDone(NStr::CStr const &_Error)
	{
		...
		// The terminal settle: a timeout still armed to watch a disconnected-state drain has
	...
	NConcurrency::TCFuture<CAsyncSocketActor::CCloseInfo> CAsyncSocketActor::f_CloseWithLinger(EAsyncSocketStatus _Status, NStr::CStr _Reason, fp64 _MaxLingerTime)
	{
...

		{
			...
			// Fully closed means the socket is gone: the shutdown has completed and nothing is
			// left to drain. A disconnected connection that still has its socket is draining
			// what was queued ahead of the shutdown the peer's half close deferred, or waiting
			// for the peer's hangup after it. Destroying it here would reset the connection
			// under that output; it lingers below like any other, and the shutdown callback
			// delivers the close info once the drain is done
			if (!Internal.m_pSocket)
			{
				CAsyncSocketActor::CCloseInfo CloseInfo;
				CloseInfo.m_Status = EAsyncSocketStatus_AlreadyClosed;
	...
	void CAsyncSocketActor::fp_Disconnect(EAsyncSocketStatus _Status, NStr::CStr const &_Reason, bool _bFatal, EAsyncSocketCloseOrigin _Origin)
	{
		...
		if (Internal.m_State == EState_Disconnected)
		{
			if (_bFatal)
			{
				// A fatal close resets, like a destroy: what the peer has not acknowledged is not
				// worth retransmitting, and the kernel lets go of the zero copy pages at once
				// rather than after the retransmissions, which a destroy would wait for
				if (Internal.m_pSocket)
					Internal.m_pSocket->f_SetAbortOnClose();
				Internal.m_pSocket.f_Clear();
		...
		{
			// The write side is cut once the queue has drained: a send still queued or with the
			// kernel would otherwise be left behind the shutdown, and fp_UpdateSend's completion
			// branch shuts down when the last of them resolves
		...
		if (_bFatal)
		{
			...
			// The fatal close resets, see the already disconnected case above
			if (Internal.m_pSocket)
				Internal.m_pSocket->f_SetAbortOnClose();
			Internal.m_pSocket.f_Clear();
			Internal.f_ShutdownDone(_Reason);
		...
		// A close with sends still queued leaves the disconnected state draining toward the
		// shutdown the deferral above owes it, and that drain makes progress only while the
		// peer reads: the timeout stays armed for it, or a peer that stops reading would wedge
		// the backlog and its promises forever. f_ShutdownDone stops the timer once the socket
		// is gone
		if (!Internal.m_pSocket || (!Internal.m_nOutgoingQueuedBytes && !Internal.m_nSendOpsInFlight))
			Internal.f_StopTimeout();

		// Close states parked behind the stream's terminal are honored now, for the reason the
		// deferral itself no longer applies: this side has closed. The re-entry is deferred to
		// the state processing already running, if this disconnect came from one
	...
	void CAsyncSocketActor::fp_StartReceiveStream()
	{
		...
		if (bStarted)
		{
			...
			// The readiness reads may have left input held in the socket; drained in a job of
			// its own, after the processing that started the stream rather than inside it
	...
	void CAsyncSocketActor::fp_SubmitSendOp(bool _bContinue, umint _iInheritedReservation)
	{
		...
		// A continuation offers no bytes and is answered yes or no; a new batch is answered with
		// what the socket took of it, 0 being the refusal
		umint nScheduled = 0;
		bool bSubmitted;
		if (_bContinue)
		...
		if (bSubmitted)
		{
			...
			// The socket took part of the batch — the TLS one seals what its outbound room allows —
			// and the transfer covers that much: the reservation shrinks to it and the rest stays in
			// the queue past the submitted bytes, where the next gather starts
			...
			// The whole gather, which is what the released functor gives back: the untaken tail is
			// counted a second time by the gather that carries it, until this release, which only
			// makes the window ask early
	...
	// Input the socket held back from the readiness reads, opened now that the stream is on: a
	// TLS socket keeps the plaintext a readiness read did not take and the records it has not
	// opened, and with no new segment to bring them out they would wait for one the peer may
	// never send
	...
	// A segment of the stream, or with _bHeldOnly the socket's held input alone, resolved the
	// same way a segment's remainder is
	void CAsyncSocketActor::fp_ReceiveStreamInput(NSys::CIoStreamSegment &&_Segment, bool _bHeldOnly)
	{
		...
		// Every segment is the peer's activity, plaintext or not: a TLS record still short of its
		// end produces nothing below, and the readiness path counts the network bytes the same way
		if (bDelivering)
			Internal.f_OnReceivedData();
		...
		// The socket's own protocol can end the stream ahead of the kernel's terminal: a TLS
		// close_notify opened from this segment's bytes. The segment is the stream's last then,
		// and the close state the socket reported must not wait for a kernel terminal that a peer
		// expecting our close_notify first never produces
	...
	void CAsyncSocketActor::fp_ProcessStateNow(NNetwork::ENetTCPState _StateAdded)
	{
		...
		)
		{
			...
			// and the stream always terminates once the peer is gone. A connection this side has
			// closed waits for nothing: there is no consumer left for those bytes, and a consumer
			// that holds delivered buffers parks every receive, so the terminal that would
			// release the close might never be reported at all
	...
	void CAsyncSocketActor::CInternal::f_UpdateTimeout()
	{
		...
		// A close that arrived with sends still queued leaves the disconnected state draining
		// toward its shutdown; that drain depends on the peer reading, so it stays under the
		// same both-directions-stale watchdog as the other non-connected states
