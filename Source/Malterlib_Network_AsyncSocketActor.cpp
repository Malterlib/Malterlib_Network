...
namespace NMib::NNetwork
{
	...
	void CAsyncSocketActor::fp_SubmitSendOp(bool _bContinue, umint _iInheritedReservation)
	{
		...
		NSys::FIoCompletion fOnComplete = [Hold = NConcurrency::CIoCompletionOpHold(Internal.m_pOpTracker), iReservation, WeakThis = fg_ThisActor(this).f_Weak()](NSys::CIoCompletion _Result) mutable
			{
				if (auto This = WeakThis.f_Lock())
		...
		NNetwork::FSocketSendReleased fOnReleased = [Hold = NConcurrency::CIoCompletionOpHold(Internal.m_pOpTracker), KeepAlives = fg_Move(KeepAlives), WeakThis = fg_ThisActor(this).f_Weak(), nGatheredBytes](umint _iTransfer) mutable
			{
