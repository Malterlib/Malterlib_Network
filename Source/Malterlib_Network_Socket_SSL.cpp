...
namespace NMib::NNetwork
{
	...
	// The TCP shutdown follows the close alert out. Under completion sends the alert waits for the
	// caller's continuation like any sealed record, and the resolve that drains it comes back here;
	// otherwise the flush inside the connection's shutdown wrote it and the socket shuts down now
	void CSocket_SSL::fp_ContinueShutdown()
	{
		if (!mp_SSLConnection.f_Shutdown())
			return;

		if (mp_SSLConnection.f_IsCompletionSend() && (mp_nSendOpsInFlight || mp_SSLConnection.f_GetPendingSend()))
			return;
	...
	void CSocket_SSL::f_OnCompletionActivated()
	{
		...
		// never resolve. Only where sends are submitted, though: a connection submitting
		// receives alone keeps the synchronous send path, and the close alert leaves through it
	...
	// Puts an operation at the end of its generation's list, once the generation its seal landed in
	// is known: the walks that resolve and release then report in submit order
	...
	// functors and leaves with the drain a release runs. Answers the plaintext sealed, which
	// is what the caller's transfer covers: the seal takes what the outbound room allows and
	// the rest stays the caller's to offer again; 0 is the connection failing
	...
	// A staged transfer keeps its own functors; they fire when the generation its seal
	// landed in has fully left
	...
	// The oldest ciphertext the transport holds goes out under the functors, which is what
	// keeps records in the order they were sealed. _iOperation names the call's own batch, or
	// -1 for a continuation that sealed nothing
	bool CSocket_SSL::fp_CarrySend(smint _iOperation, NSys::FIoCompletion &&_fOnComplete, FSocketSendReleased &&_fOnReleased)
	{
		...
		{
			// A submit that fails does so terminally; the call's transfer record — if it made
			// one — was failed along with every other by the submit path
		...
		// stays recorded and leaves with the next operation
		if (_iOperation >= 0)
		{
	...
	bool CSocket_SSL::fp_SubmitPinnedSend(void const *_pData, umint _nBytes, umint _iBuffer, NSys::FIoCompletion &&_fOnComplete, FSocketSendReleased &&_fOnReleased)
	{
		...
		umint nScheduled = mp_Socket.f_SubmitSendVectored
			(
				...
				, 1
				,
				[fOnComplete = fg_Move(_fOnComplete), iBuffer = _iBuffer]
				...
				[fOnReleased = fg_Move(_fOnReleased), pKeepAlive = mp_SSLConnection.f_GetPinnedKeepAlive(_iBuffer), iBuffer = _iBuffer]() mutable
				{
					...
					// back to this socket for the generation it belongs to. The keep alive rides
					// here: the release is when the kernel is done with the generation's buffer,
					// which for a zero copy send is later than the completion
					fOnReleased(iBuffer);
				}
			)
		;
		...
		// A generation goes out whole: the loop takes any single span this size, so one it took
		// only part of is a broken transport. The operation it did make still completes and
		// releases the pin, so the hold stays with it
		if (nScheduled)
		{
			ReleaseHold.f_Clear();
			++mp_nSendOpsInFlight;
	...
	// staged generation still needs an operation to carry it
	bool CSocket_SSL::f_ResolveSend(NMib::NSys::CIoCompletion &_Result)
	{
		...
		// Every scheduled byte left: the generation is drained and its transfers resolve
		...
		// accumulates across the continuations, whose own calls hand over nothing
		mp_nSendPlaintextHeld += nCarrierPlaintext;

		// A seal that failed part way left the connection broken behind the ciphertext it did
		// produce; with that out, the broken state is what the socket reports, and no
		// continuation is begun on a connection that seals nothing more
		if (mp_SSLConnection.f_BrokenState())
		{
		...
		// Staged generations still need carrying; the caller answers false with a continuation,
		// whose fresh functors ride the next operation. But only when one can actually be
		// begun: pending bytes parked behind pinned generations
	...
	// The peer's close_notify ends the stream, and so does a record layer that has failed, on a
	// bad record or a fatal alert: nothing further can be opened, and a peer that keeps the
	// transport open after that must not keep the connection waiting for its end
	...
	void CSocket_SSL::f_SetSendWindow(umint _nBytes, bool _bConfigured)
	{
		...
		// armed the transport; the ceiling takes as long as nothing is pinned, and otherwise
		// waits for the release that unpins the last generation
		mp_bSendWindowPending = false;
